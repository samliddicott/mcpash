...
| Completion/readline interactive semantics (`bind/complete/compgen/compopt`) | partial | `tests/diff/cases/bash-builtin-completion.sh`, `tests/compat/run_completion_interactive_matrix.sh` | Non-interactive behavior and PTY marker checks are covered; full readline/completion ecosystem parity remains out of scope for this milestone. |
