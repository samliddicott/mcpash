...
| Interactive foreground SIGINT continuation (no-trap Ctrl-C) | covered | `tests/compat/run_interactive_sigint_matrix.sh` | Strict deterministic lane now validates foreground-command SIGINT interruption + shell continuity. Literal PTY `^C` lane remains informational due environment-dependent control-character delivery. |
...
| TMOUT interactive auto-logout | covered | `tests/compat/run_interactive_tmout_matrix.sh` | Interactive idle timeout/auto-logout behavior is covered with dedicated PTY evidence. |
