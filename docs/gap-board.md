...
| Parameter transformation operators `${v@op}` | partial | `tests/diff/cases/bash-man-param-transform-ops.sh`, `tests/diff/cases/bash-man-param-transform-ops-variants.sh`, `tests/diff/cases/bash-man-param-transform-prompt.sh` | Core scalar/positional/array and prompt-transform lanes now run in bash-compat mode; remaining partiality is deeper `${v@P}` edge semantics (notably `\\w`/tilde behavior) and wider prompt-escape coverage. |
| TIMEFORMAT semantics | partial | `tests/diff/cases/bash-man-timeformat.sh` | `%P` and full `TIMEFORMAT` behavior differ from bash in dedicated differential case. |
