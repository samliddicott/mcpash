...
| Sentinel collision proof cases | partial | `tests/diff/cases/unicode-pua-literals.sh`, `tests/diff/cases/unicode-pua-case-assignment.sh`, `tests/diff/experimental/unicode-pua-operators.sh` | Keep the two passing collision cases in the parity lane; resolve `unicode-pua-operators` mismatch before promotion. |
