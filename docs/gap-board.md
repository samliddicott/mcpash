...
| Parameter transformation operators `${v@op}` | partial | `tests/diff/cases/bash-man-param-transform-ops.sh`, `tests/diff/cases/bash-man-param-transform-ops-variants.sh` | Core scalar/positional/array transform forms now run in bash-compat mode; remaining partiality is deeper `${v@P}` prompt-expansion semantics and extended edge coverage. |
