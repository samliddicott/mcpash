...
| Feature | Required behavior | Conformance tests | Implementation hints | Status |
|---|---|---|---|---|
| `sh.vars` mapping | live get/set/del/iter/contains/len | CRUD and iteration tests | map directly to shell var store APIs | todo |
| `sh.env` mapping | exported env view with set+export on write | read exported vars; write exports | filter by export attr | todo |
...
| Feature | Required behavior | Conformance tests | Implementation hints | Status |
|---|---|---|---|---|
| `sh.fn` map | get/set/del/iter/contains | body read/write/delete tests | hook into shell function table | todo |
| `sh.fn.call` | callable namespace invocation | `sh.fn.foo("x")` returns stdout | invoke shell func with captured stdout | todo |
...
| Feature | Required behavior | Conformance tests | Implementation hints | Status |
|---|---|---|---|---|
| `sh()` | run shell command, return stdout, raise on non-zero | success + failure metadata tests | lightweight wrapper over run/check/capture | todo |
| `sh.run()` core | completed object with args/returncode/stdout/stderr | check modes + capture flags | model after `subprocess.run` | todo |
| `check=True` | raise `ShellCalledProcessError` on non-zero | exit 7 raises | include returncode/cmd/stdout/stderr | todo |
| stdio routing | support PIPE/STDOUT/DEVNULL | routing correctness tests | normalize stream config object | todo |
| `sh.popen()` | process handle with poll/wait/communicate + context manager | lifecycle tests | retain child handle + IO endpoints | todo |
...
| `sh.tie/untie` | python-side getter/setter ties | getter invoked on read, setter on write | tie registry keyed by var name | todo |
...
| `sh.shared` mapping | Python access to shared backend | read/write/delete from Python | expose same storage via mapping API | todo |
